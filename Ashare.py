...
DEFAULT_TIMEOUT = 10
DEFAULT_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AshareClient/1.0"
}


def _http_get(url, timeout=DEFAULT_TIMEOUT, headers=None):
    """安全封装的 HTTP GET 请求，带超时和状态码检查。"""
...
def get_price_day_tx(code, end_date='', count=10, frequency='1d'):     #日线获取
    ...
    st = json.loads(_http_get(URL).content);    ms='qfq'+unit;      stk=st['data'][code]
...
def get_price_min_tx(code, end_date=None, count=10, frequency='1d'):    #分钟线获取
    ...
    st = json.loads(_http_get(URL).content);       buf=st['data'][code]['m'+str(ts)]
...
def get_price_sina(code, end_date='', count=10, frequency='60m'):    #新浪全周期获取函数
    ...
    dstr = json.loads(_http_get(URL).content);
