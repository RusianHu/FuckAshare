...
### 🗓️ 分红日历
- 扫描未来 7/14/30 日或最多 60 日自定义窗口内的股权登记事件
- 默认仅显示实施分配，支持沪/深/北市场、未确认方案、收益率与税档筛选
- 使用当前行情重新计算单次毛现金率，以及个人持有不超 1 月、1 月至 1 年、超过 1 年的税后估算
- 详情抽屉展示近 10 年分红时间线与近 5 年现金统计
...
├── dividend_feature_tests.php # 分红功能、真实数据及 loopback 聚焦测试
...
│   ├── DividendDataProvider.php # 分红数据源契约
│   ├── EastmoneyDividendClient.php # 东方财富分红事件适配与独立熔断
│   └── DividendService.php # 事件、行情、税率、缓存与详情聚合
...
| `market_api.php` | 统一行情 API 入口 | `action`=quote/kline/hot_stock/screener/fundx/stock_flow/sector_flow/hot_stocks/market_breadth/dividend_calendar/dividend_detail |
