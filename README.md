...
<img width="1272" height="1234" alt="image" src="https://github.com/user-attachments/assets/083f9332-f119-4f46-bef4-d45e0c0c692d" />
...
### 📰 新闻舆情
- 支持 **指定股票 / 指定基金 / 市场热点** 三种查询模式；代码会自动映射股票或基金名称，并合并“代码 + 名称”搜索结果
- 行情工作台、基金详情与分红详情抽屉内置可复用的“标的舆情脉搏”；实时看板、自选股和热榜跳转聚焦股票后会自动继承联动
- 嵌入卡片展示最近标题、标题情绪、样本与来源覆盖，支持刷新、进入完整舆情和携带当前标的一键 AI 研判；前端按标的缓存并防止快速切换时旧响应覆盖
- 市场模式支持最多 4 个中文关键词并行检索，跨关键词去重后按发布时间倒序展示
- 页面、公开 API 与 AI 工具严格只返回 **标题、来源、发布时间、原文链接**，不透传新闻正文
- 基金新闻执行严格相关性过滤，宁可少报也不把泛基金资讯冒充为指定基金新闻
- 提供基于中文标题词典与时间衰减的确定性情绪快照，输出正/中/负样本数、得分、置信度和标题证据；明确标记为弱信号
- 默认使用东方财富组合 Provider：公开搜索正常时保持原结果；海外出口 callback 被重写且只返回 `passportWeb` 时识别为地域能力裁剪，不累计搜索熔断失败
- 股票/市场路由为“站内搜索 → 个股 F10 公司资讯 → 7×24 快讯”；基金在站内搜索被海外地域裁剪时，改走“Google News RSS 媒体报道 + 东方财富基金公告”专用回退
- 基金回退只在 `asset_type=fund` 启用，代码与名称继续执行严格相关性过滤；不会把泛市场新闻、股票 F10 数据或同号股票内容冒充基金新闻
- 新闻服务具备独立缓存、负缓存与 stale 降级，API 元数据保留实际 Provider、路由原因和各源状态，便于海外部署排障
...
- AI 可主动调用行情、K线、资金流、板块、市场宽度、新闻舆情、基金、雪球热度/选股、技术指标等只读研究工具
...
- **响应式布局**：完美适配桌面端和移动端
- **模块化Tab页**：股票行情、实时看板、板块资金、分红日历、新闻舆情、雪球洞察、基金分析等模块
...
| 新闻 PoC 数据源 | 东方财富公开搜索 / 个股 F10 / 7×24；基金海外回退使用 Google News RSS 与东方财富基金公告（均仅输出标题、来源、时间、链接） |

> ⚠️ **数据授权**：股票与基金分红事件、行情、净值及新闻 PoC 均包含东方财富公开页面接口，属于未承诺 SLA 的公开数据源。本项目仅供研究学习；公开商业化展示或再分发前，需自行确认数据使用与再分发授权。新闻链路不输出正文并不能替代授权确认。
...
├── news_feature_tests.php # 新闻映射、字段白名单、情绪算法、AI Tool 与真实数据测试
...
├── news_api.php           # 股票/基金/市场新闻与标题情绪快照只读 API
...
│   ├── NewsDataProvider.php # 可替换新闻数据源契约
│   ├── EastmoneyNewsClient.php # 东方财富新闻搜索、动态 JSONP callback、地域能力识别与独立熔断
│   ├── EastmoneyF10NewsClient.php # 东方财富个股 F10 公司资讯与精确股票代码补充
│   ├── EastmoneyFastNewsClient.php # 东方财富 7×24 快讯、标题/关联证券匹配与四字段输出
│   ├── FundNewsClient.php # 基金专用 Google News RSS + 东方财富公告海外回退
│   ├── CompositeNewsProvider.php # 搜索、个股 F10、快讯的三段能力路由与源状态记录
│   └── NewsService.php # 标的映射、跨关键词去重、缓存与确定性标题情绪快照
...
| `news_api.php` | 新闻与舆情只读 API | `action`=asset/market/sentiment；标的查询用 `asset_type`、`code`/`name`，市场查询用 `keywords`（最多4个），最大 `limit`=50 |
...
| `fa_get_asset_news` | 查询指定股票/基金新闻；自动映射名称，仅返回四个公开字段 |
| `fa_get_market_hot_news` | 聚合最多四个市场/板块/主题关键词的近期新闻 |
| `fa_get_sentiment_snapshot` | 从新闻标题生成确定性、可解释的弱情绪快照 |
...
.\php\php.exe news_feature_tests.php --live
```

`ai_tool_runtime_tests.php` 覆盖 curl_multi 真实错误码、整批传输失败停止、参数规范化去重、分红工具自动参数、否定语境护栏及中证全收益基准选择。`dividend_feature_tests.php` 默认覆盖换算、三档税率、状态、沪深北识别、排序分页、缺行情、stale 缓存及 AI Tool schema/参数；`--live` 连接真实东方财富事件与行情；`--loopback` 验证内部 token、两个新工具和并行 HTTP 执行。`fund_dividend_feature_tests.php` 额外覆盖基金分红事件聚合工具的 ETF 解析、NAV/日 K/基准对齐、折溢价白名单、确定性收益与恢复指标、公告证据等级、部分/全部失败语义；`--live` 连接真实基金分红与中证指数链路，`--loopback` 验证基金分红档案和事件聚合工具的内部执行。`news_feature_tests.php` 覆盖东方财富 JSONP、HTTPS 链接、正文不外泄、股票/基金名称映射、基金相关性过滤、市场去重排序、情绪算法、三个 strict AI Tool 和前端契约；`--live` 连接真实股票、基金与市场新闻。
