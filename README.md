...
- AI 可主动调用行情、K线、资金流、板块、基金、雪球热度/选股、技术指标等只读研究工具
...
- 股票研究可调用实时行情、K线、资金流、板块流向、热股榜、雪球热度和条件选股
...
│   ├── AIChatToolAgent.php # Chat Completions 工具调用编排器
...
- `stream_after_tool_round=true` 时，模型主动选择一轮工具后直接生成最终回答，避免兼容端点多轮工具握手卡顿
- `auto_prefetch=true` 仅作为兜底：当模型没有调用工具或工具握手失败时，股票/基金分析请求才会由服务端确定性预取核心数据
...
`phase2_core_tests.php` 覆盖缓存、熔断和核心服务层行为；`ai_tool_agent_tests.php` 覆盖工具 schema、参数拒绝、工具输出截断、指标计算、工具编排和普通 SSE 回退。
