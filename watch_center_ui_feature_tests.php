...
checkContract(strpos($script, 'items.filter(isMarketItem)') !== false, '股票与场内 ETF 统一走交易所行情路由');
checkContract(strpos($script, 'items.filter(isOtcFund)') !== false, '只有场外基金进入估值/净值路由');
checkContract(strpos($script, "valueKind: 'market'") !== false, '交易所行情明确标记为现价口径');
checkContract(strpos($script, 'dataAt: q.quote_time') !== false, '场内行情使用上游真实行情时间');
checkContract(strpos($script, 'dataAt: f.gztime || f.jzrq') !== false, '基金使用估值时间或净值日期');
checkContract(strpos($script, "at: nowTimeStr()") === false, '不得再用浏览器接收时间冒充数据时间');
checkContract(strpos($script, '日涨幅') !== false, '官方净值涨幅必须明确标成日涨幅');
checkContract(strpos($script, 'data-instrument-type') !== false, '添加资产时保留场内/场外标的分类');
