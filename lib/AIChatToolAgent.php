    ...
    private function systemPrompt(?AIAgentProfile $profile = null, bool $includeToolInstructions = true): string
    {
        ...
        if ($includeToolInstructions) {
            $lines = array_merge($lines, [
                ...
                '必须区分三种价格口径：场内 ETF 的交易所现价用 fa_get_stock_quote；场外基金盘中估值用 fa_get_fund_estimate；官方单位净值带明确日期。fa_get_fund_estimate 返回 quote_type=latest_nav 或 estimate_available=false 时，仅表示盘中估值不可用并降级到官方净值，严禁称为实时估值、现价或当日盘中涨幅。',
